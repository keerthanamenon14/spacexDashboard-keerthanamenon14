...
export default function LaunchDetails() {
  ...
  const launchDetailsInfo = useSelector((state) => state.launchdetails.data);
  ...
  function filterData(launchFilterOpt, dateFilterOpt) {
    ...
    if (launchDetailsInfo && (launchFilterOpt || dateFilterOpt)) {
      ...
      if (
        (launchFilterOpt == "All Launches" || launchFilterOpt == "null") &&
        dateFilterOpt == null
      ) {
        setLaunchDetails(launchDetailsInfo);
        setLaunchCount(launchDetailsInfo.length);
        ...
        filteredData = launchDetailsInfo.filter(
          (item) =>
            (menuOptions[[item.launch_success]] == launchFilterOpt ||
              launchFilterOpt == "" ||
  ...
  useEffect(() => {
    if (launchDetails) {
      ...
      if (
        launchDetails.length < rulesPerPage ||
        launchDetails.length < indexOfFirstRule
      ) {
        setcurrentPage(1);
        setPage(1);
        ...
        <Grid item className={classes.detailsTable}>
          <TableContainer component={Paper}>
            <Table className={classes.table} aria-label="simple table">
              ...
              <TableBody>
                {launchDetailsLoading && <SkeletonTableList number={7} />}
                {currentDetail &&
                  currentDetail.map((row, index) => (
                    <TableRow key={index}>
                      <TableCell className={classes.tableData}>
                        {row.flight_number}
                      </TableCell>
